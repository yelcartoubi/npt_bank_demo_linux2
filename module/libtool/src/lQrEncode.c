...
static int GetEncodeVersion(DataInfo* stDataInfo);
static int EncodeSourceData(DataInfo* stDataInfo, int nVerGroup);
static int GetBitLength(unsigned char nMode, int ncData, int nVerGroup);
static int SetBitStream(int nIndex, unsigned short wData, int ncData);
...
static unsigned char AlphabetToBinaly(unsigned char c);
static unsigned short KanjiToBinaly(unsigned short wc);
static void GetRSCodeWord(unsigned char* lpbyRSWork, int ncDataCodeWord, int ncRSCodeWord);
...
static void LcdToBmp(const char *lcdBuf,char *bmpBuf, int w, int h);
static char* BmpZoomIn(int ntimes);
...
int EncodeSourceData(DataInfo* stDataInfo, int nVerGroup)
{
	...
	for (m_ncDataBlock = i = 0; i < stDataInfo->nLen; ++i)
	{
		unsigned char byMode;

		if (i < stDataInfo->nLen - 1 && IsKanjiData(stDataInfo->szInputData[i], stDataInfo->szInputData[i + 1]))
			byMode = QR_MODE_KANJI;
		else if (IsNumeralData(stDataInfo->szInputData[i]))
			byMode = QR_MODE_NUMERAL;
		else if (IsAlphabetData(stDataInfo->szInputData[i]))
	...
	for (i = 0; i < m_ncDataBlock && m_ncDataCodeWordBit != -1; ++i)
	{
		if (m_byBlockMode[i] == QR_MODE_NUMERAL)
		{
			...
			for (j = 0; j < m_nBlockLength[i]; j += 3)
			{
				...
				{
					wBinCode = (unsigned short)(((stDataInfo->szInputData[ncComplete + j]	  - '0') * 100) +
									  ((stDataInfo->szInputData[ncComplete + j + 1] - '0') * 10) +
									   (stDataInfo->szInputData[ncComplete + j + 2] - '0'));

					m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, wBinCode, 10);
				}
				else if (j == m_nBlockLength[i] - 2)
				{
					wBinCode = (unsigned short)(((stDataInfo->szInputData[ncComplete + j] - '0') * 10) +
									   (stDataInfo->szInputData[ncComplete + j + 1] - '0'));

					m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, wBinCode, 7);
				}
				else if (j == m_nBlockLength[i] - 1)
				{
					wBinCode = (unsigned short)(stDataInfo->szInputData[ncComplete + j] - '0');
		...
		else if (m_byBlockMode[i] == QR_MODE_ALPHABET)
		{
			...
			for (j = 0; j < m_nBlockLength[i]; j += 2)
			{
				...
				{
					wBinCode = (unsigned short)((AlphabetToBinaly(stDataInfo->szInputData[ncComplete + j]) * 45) +
									   AlphabetToBinaly(stDataInfo->szInputData[ncComplete + j + 1]));

					m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, wBinCode, 11);
				}
				else
				{
					wBinCode = (unsigned short)AlphabetToBinaly(stDataInfo->szInputData[ncComplete + j]);
		...
		else if (m_byBlockMode[i] == QR_MODE_8BIT)
		{
			...
			{
				m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (unsigned short)stDataInfo->szInputData[ncComplete + j], 8);
			}
		...
		else // m_byBlockMode[i] == QR_MODE_KANJI
		{
			...
			{
				unsigned short wBinCode = KanjiToBinaly((unsigned short)(((unsigned char)stDataInfo->szInputData[ncComplete + (j * 2)] << 8) + (unsigned char)stDataInfo->szInputData[ncComplete + (j * 2) + 1]));
...
{
	int nVerGroup = stDataInfo->nVersion >= 27 ? QR_VRESION_L : (stDataInfo->nVersion >= 10 ? QR_VRESION_M : QR_VRESION_S);
	int i, j;

	for (i = nVerGroup; i <= QR_VRESION_L; ++i)
	{
		if (EncodeSourceData(stDataInfo, i))
		{
			if (i == QR_VRESION_S)
			{
				for (j = 1; j <= 9; ++j)
				{
					if ((m_ncDataCodeWordBit + 7) / 8 <= QR_VersonInfo[j].ncDataCodeWord[stDataInfo->nLevel])
						return j;
				}
			}
...
*/
int PubEncodeDataAndGenerateBmp(DataInfo* stDataInfo, BmpInfo* stBmpInfo)
{
	int i, j;

	//PubAsciiToUtf8(stDataInfo);
	m_nLevel = stDataInfo->nLevel;
	m_nMaskingNo = stDataInfo->nMask;

	int ncLength = stDataInfo->nLen;

	if (ncLength == 0)
		return APP_FAIL;
	
	int nEncodeVersion = GetEncodeVersion(stDataInfo);

	if (nEncodeVersion == 0)
		return APP_FAIL;

	if (stDataInfo->nVersion == 0)
	{
		m_nVersion = nEncodeVersion;
	}
	else
	{
		if (nEncodeVersion <= stDataInfo->nVersion)
		{
			m_nVersion = stDataInfo->nVersion;
		}
	}

	int ncDataCodeWord = QR_VersonInfo[m_nVersion].ncDataCodeWord[stDataInfo->nLevel];
	...
	int ncBlock1 = QR_VersonInfo[m_nVersion].RS_BlockInfo1[stDataInfo->nLevel].ncRSBlock;
	int ncBlock2 = QR_VersonInfo[m_nVersion].RS_BlockInfo2[stDataInfo->nLevel].ncRSBlock;
	int ncBlockSum = ncBlock1 + ncBlock2;

	int nBlockNo = 0;

	int ncDataCw1 = QR_VersonInfo[m_nVersion].RS_BlockInfo1[stDataInfo->nLevel].ncDataCodeWord;
	int ncDataCw2 = QR_VersonInfo[m_nVersion].RS_BlockInfo2[stDataInfo->nLevel].ncDataCodeWord;
	...
	int ncRSCw1 = QR_VersonInfo[m_nVersion].RS_BlockInfo1[stDataInfo->nLevel].ncAllCodeWord - ncDataCw1;
	int ncRSCw2 = QR_VersonInfo[m_nVersion].RS_BlockInfo2[stDataInfo->nLevel].ncAllCodeWord - ncDataCw2;
...
///////////////////////////////////////////////
	...
	stBmpInfo->xsize = m_nSymbleSize*N_TIMES;
	stBmpInfo->ysize = m_nSymbleSize*N_TIMES;
	LcdToBmp(BmpZoomIn(ntimes),stBmpInfo->bmpbuff, stBmpInfo->xsize, stBmpInfo->ysize);
	
	return APP_SUCC;
}

/**
* @brief Enlarge the lattice data and output in one-dimensianal array
* @param [in] times of enlarge	
* @return
* @li one-dimensianal array of lattice data
*/
char* BmpZoomIn(int ntimes)
{
	int i,j,xx,yy;
	char *p;
	char tmpbuf[m_nSymbleSize*ntimes+1][m_nSymbleSize*ntimes+1];
	char data[m_nSymbleSize*ntimes*m_nSymbleSize*ntimes+1];
	...
	for (i = 0; i < m_nSymbleSize; i++)
	{
		for (j = 0; j < m_nSymbleSize; j++)
		{
			for(xx = 0; xx < ntimes; xx++)
			{
				for(yy = 0; yy < ntimes; yy++)
				{
					tmpbuf[i*ntimes+xx][j*ntimes+yy] = m_byModuleData[i][j];//zoom in N_TIMES size
				}
			}	
		}
	}

	for (i = 0; i < m_nSymbleSize*ntimes; ++i)
	{
		for (j = 0; j < m_nSymbleSize*ntimes; ++j)
		{
			data[i*m_nSymbleSize*ntimes+j] = tmpbuf[j][i];
		}	
	}
	return p;
}
...
/**
* @brief Change LCD buffer to BMP
* @param [in] LCD buffer	
* @param [in] width
* @param [in] height
* @param [out] BMP buffer
* @return
* @li 
*/
...
//				mask1 =1;
		...
		for(j=0;(j<8)&&(k<(w+((i/((w+7)>>3))/*/8*/)*w));j++,k++)
		{
			if(j&7)
				mask2 = mask2>>1;
			else 
...
void PubAsciiToUtf8(DataInfo* stDataInfo)
{
	...
	strcpy(sztmp,stDataInfo->szInputData);
	nlen = stDataInfo->nLen;
	stDataInfo->nLen = 0;
	memset(stDataInfo->szInputData,0,sizeof(stDataInfo->szInputData));

	SwithToUtf_8((unsigned char*)sztmp,nlen,(unsigned char*)stDataInfo->szInputData,&(stDataInfo->nLen));
}
